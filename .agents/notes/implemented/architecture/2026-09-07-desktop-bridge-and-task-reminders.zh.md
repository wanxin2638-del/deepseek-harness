...
Windows 桌面壳（[apps/desktop](../../../../apps/desktop/README.md)，Plan 1）承载 web profile；本次改动之前，它没有任何 OS 级注意力原语。任务状态变化只在窗口聚焦时到达用户：完成的会话点亮侧边栏绿色"完成"标记，挂起的审批只能在 composer 接管内应答。窗口失焦时完成被无声略过、审批无人应答。新能力是"任务状态变化 → 用户可见提醒"，壳只提供三个原语 —— toast、任务栏闪烁、窗口状态 —— 不含任何产品语义。
