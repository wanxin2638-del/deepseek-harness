...
CI does not restore or save a lint-result cache. `DSH_OXLINT_THREADS` makes the shared runner pass the same bound to Oxlint's `--threads` option and the type-aware backend's `GOMAXPROCS` environment variable; ordinary local runs use both defaults. Explicit staged-file checks use project-free Oxlint validation and safe fixes with one bounded retry and accept selections containing only ignored files. Public `lint` and CI retain the complete type-aware rules after preparing generated declarations.
