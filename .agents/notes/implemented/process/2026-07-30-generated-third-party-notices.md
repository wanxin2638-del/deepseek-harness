...
**Freshness is maintained, not merely enforced.** The committed bytes are asserted inside [`scripts/gen-third-party-notices.spec.ts`](../../../../scripts/gen-third-party-notices.spec.ts), which the test lane already runs. `pnpm run verify-third-party-notices` remains available for a standalone check.
...
A dependency edit requires the notices generator or its verification command before commit. The test lane also reports stale output with the command needed to repair it.
