...
`*.i18n.yaml` 使用由仓库维护的 `dsh-translation-pairing` 合并驱动。`scripts/install-translation-pairing.mjs` 会将其命令注册到 worktree 本地的 Git 配置中，因为受跟踪的属性可以指定驱动，却无法携带其可执行命令。
...
CI 中全语料的 `doc-sync`（文档同步门禁）会对照两侧文档的精确字节，校验已提交的 `.i18n.yaml` 文件，并继续作为权威检查。
...
|---|---|---|
| 全新安装无法探测或配置驱动 | 不会发布新的驱动配置；任何新增的 worktree 值都会回滚。 | 恢复依赖并重新运行 `node scripts/install-translation-pairing.mjs`。 |
...
脚本测试通过已安装的启动器覆盖无冲突组合，并覆盖运行时缺失与入口点损坏时的文本回退、安装脚本探测回滚、从含未解决条目的索引中显式恢复、可安全解决的配对与配对文档冲突的配对并存、被编辑过的伴随文件、非文本默认合并配置、记录解析和 worktree 本地安装。既有的全语料校验器继续证明已提交的记录与两侧配对文档相符。
...
安装脚本会在 worktree 配置中预留 `merge.dsh-translation-pairing.*`，并拒绝与之冲突的自定义值。自动组合依赖已安装的 Node 依赖；运行时不可用时会产生可见的未解决文本结果，而不是选择陈旧元数据。
