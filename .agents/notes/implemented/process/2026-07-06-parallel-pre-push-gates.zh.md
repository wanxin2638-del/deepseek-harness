...
本记录中的本地钩子部分由[移除 Lefthook 本地钩子](2026-09-08-remove-lefthook-local-hooks.zh.md)移除。有界门禁调度器和包级 `publint` 并行机制仍用于 CI、`doc-sync` 和显式本地命令。调度器的快速失败选项记录在[门禁运行器快速失败](2026-08-27-gate-runner-fail-fast.zh.md)。
