...
Chat 在对话与轨迹视图旁注册一个 `上下文` 标签。它读取 `SessionFace.readContext()`，展示 Host 返回的精确 `asOfSeq`、已记录的 request header 与 canonical message history。Session event window 版本变化后，视图会重新读取；较早的请求晚返回时会保留更新的结果，并使用共享 JSON tree 的复制操作。该视图只用于检查模型输入，不修改 Session，也不会在 Client 再组装一份历史。
