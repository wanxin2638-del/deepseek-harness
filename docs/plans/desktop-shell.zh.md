...
| P1.6 打包 | 已完成 | 2026-09-07 | `electron-builder.yml` + `assemble` 扩展（`.runtime-pack/` 嵌套 payload、`.runtime-node/` node.exe 暂存、探针 home 移出 payload）；`portable.useZip: true` 单步内嵌规避提取竞态；产物 `DeepSeek Harness Desktop 0.1.3-alpha.1.exe` 220.6 MB；依赖/竞态/readiness 修补见 [verification-shell.md §8](notes/verification-shell.zh.md) |
| P1.7 冷启动验证 | 已完成 | 2026-09-07 | 见 [notes/verification-shell.zh.md](notes/verification-shell.zh.md)；冷启动/生命周期/用户数据/无 key 路径/复用/端口占用/体积逐项取证 |
